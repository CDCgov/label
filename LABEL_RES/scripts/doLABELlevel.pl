...
if ( scalar(@ARGV) < 2 ) {
    ...
    $message .=
      "\t\t-C|--custom-reverse <path>\tUse custom reverse corrected viterbi model. Path contains equivalent pHMMs.\n";
    die( $message . "\n" );
}
...
if ( !$bestScoreMode ) {
    ...
    # READ shogun results.
    $/ = "\n";
    open( IN, '<', "$ppath/$testResults" )
      or die("$0 ERROR: result file \"$ppath/$testResults\" was not written by SHOGUN.\n");
    $line = <IN>;
    chomp($line);
    @results = split( / /, $line );
    close(IN);
